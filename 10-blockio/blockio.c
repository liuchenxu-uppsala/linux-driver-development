...
void init_device(void) {
    ...
    // 阻塞等待
    init_waitqueue_head(&led_device.head);
}
...
ssize_t read(struct file * f, char __user *buff, size_t n, loff_t *l) {
    ...
    # if 0
    wait_event_interruptible(&led_device.head,led_device.key_release_value>0);
    #endif
    DECLARE_WAITQUEUE(wait_, current);
    add_wait_queue(&led_device.head,&wait_);
    set_current_state(TASK_INTERRUPTIBLE);
    schedule();
    set_current_state(TASK_RUNNING);
    printk("led_driver_read continue");
    remove_wait_queue(&led_device.head,&wait_);
    char* return_value = "Hello";
    copy_to_user(buff,return_value,sizeof(*return_value));
    return 0;
}
