...
void timerFunction(unsigned long addr) {
    ...
    if (key_value == 0) {
        ...
        if (led_device.key_current_value == 0) {
            ...
            kill_fasync(&led_device.queue_fasync,SIGIO,POLL_IN);
        }
    }
...
void init_device(void) {
    ...
    request_irq(led_device.irq_number, irq_deal,IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_SHARED, "key_irq",
                &led_device);
    led_device.key_current_value = -1;
...
int fsync_fun(int fd, struct file *filp, int on) {
    printk("fsync");
    fasync_helper(fd, filp, on, &led_device.queue_fasync);
}
